...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron"
  ...
  <sch:pattern id="acceptance-lineage-consistency">
    ...
    <sch:rule context="p:pxml[p:meta/p:doc_class='review_sidecar']">
      <sch:assert test="not(p:payload/p:acceptance_lock_sha256) or normalize-space(p:payload/p:acceptance_lock_sha256) != ''">E701_REVIEW_ACCEPTANCE_HASH_EMPTY: review_sidecar acceptance_lock_sha256 cannot be empty when provided.</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="p:pxml[p:meta/p:doc_class='execution_trace']/p:payload/p:events/p:event[p:event_type='review_done']">
      <sch:assert test="normalize-space(p:lineage_lock_sha256) != ''">E704_TRACE_REVIEW_DONE_LINEAGE_REQUIRED: review_done event must include lineage_lock_sha256.</sch:assert>
    </sch:rule>
